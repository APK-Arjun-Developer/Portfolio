...
export const projects: Project[] = [
  {
    ...
    techStack: ['.NET 10', 'ASP.NET Core', 'Entity Framework Core 10', 'SQL Server', 'JWT', 'Serilog', 'MailKit + Polly', 'Swagger / OpenAPI'],
    challenges: [
      {
        problem: 'Tenant filtering needed to be impossible to forget — a single missed TenantId filter would leak data across tenants.',
        solution: 'Built a generic EF Core global query filter, applied via reflection to every entity implementing ITenantEntity + IAuditableEntity at model-build time, sourced from a scoped ICurrentTenantService populated from the JWT tenant_id claim by TenantMiddleware — isolation lives in one place, not in every query.',
      },
      {
        problem: 'Baking permissions into the JWT would make them stale: revoking a role would not take effect until the token expired, and the token would balloon as permission sets grew.',
        solution: 'Kept permissions out of the JWT entirely — it only carries user_id, tenant_id, and role_id(s). Every request resolves the caller\'s current permission set through a cached PermissionService (TTL-based, invalidated on role change), so access changes apply on the very next request.',
      },
      {
        problem: 'One Identity schema needed to support both a system-level SuperAdmin (no tenant) and tenant-scoped admins/users without hardcoded role-name checks leaking into controllers.',
        solution: 'Added a RoleScope enum (System vs Tenant) and ASP.NET Core authorization policies (SystemAdminOnly, TenantAdminOrAbove, AuthenticatedTenantUser) that assert on claims and scope instead of string role names, keeping the SuperAdmin\'s tenant_id = Guid.Empty sentinel contained to one place.',
      },
    ],
  ...
  },
  {
    ...
    techStack: ['.NET 10', 'ASP.NET Core MVC', 'Entity Framework Core 10', 'SQL Server', 'ASP.NET Core Identity', 'QuestPDF', 'ClosedXML', 'Bootstrap 5 / AdminLTE'],
    challenges: [
      {
        problem: 'Four roles query the same FuelTransaction table but must only see their own slice — Admin sees everything, an AirlineExecutive only their airline\'s transactions, a FuelSupplyExecutive only their company\'s, a FuelCoordinator only their airport\'s.',
        solution: 'Centralized the scoping in one place: FuelTransactionService.GetFilteredTransactionsQueryAsync reads the caller\'s roles and applies the matching filter (AirlineId / FuelCompanyId / AirportId, sourced from fields on ApplicationUser) before history, details, invoice, or export queries run — every read path goes through the same guarded entry point instead of repeating the filter logic.',
      },
      {
        problem: 'Transaction numbers needed to be unique and human-readable, but concurrent creates could collide on the unique index under load.',
        solution: 'Generated numbers as a timestamp plus random suffix (FT-yyyyMMddHHmmss-XXXXXX) and wrapped the insert in a retry loop (up to 3 attempts) that catches the DbUpdateException from a collision and regenerates, rather than relying on a database sequence.',
      },
      {
        problem: 'Deleting a fuel transaction needed to be reversible and auditable — financial records should never simply disappear.',
        solution: 'Modelled deletion as soft-delete: an EF Core global query filter (HasQueryFilter(x => !x.IsDeleted)) hides deleted rows from every query automatically, while IsDeleted/DeletedAt/DeletedBy fields keep the row and its audit trail intact in the database.',
      },
    ],
  ...
  },
  {
    ...
    techStack: ['React 18/19', 'TypeScript 5.8', 'Material UI (v5–v9)', 'React Hook Form', 'Zod', 'tsup', 'Vitest', 'Storybook'],
    challenges: [
      {
        problem: 'The package must not ship its own MUI, React, or form libraries, yet still needs full type safety against them during development — and optional features like react-window or MUI\'s DatePicker shouldn\'t force every consumer to install them.',
        solution: 'Declared @mui/material, react, react-hook-form, @hookform/resolvers, and zod as peerDependencies (mirrored as devDependencies for local builds and tests). The tsup build externalizes all of them, plus react-window and the MUI date-picker packages, which are optional peer dependencies only needed when those specific features are used.',
      },
      {
        problem: 'Some consumers need a field type the library does not ship — most commonly a MUI DatePicker, which pulls in @mui/x-date-pickers as an extra dependency the library can\'t assume is installed.',
        solution: 'Built a module-level field registry mapping a type string to a render component, exposed via registerFieldType(type, Component). Consumers call it once at startup to add their own renderer for any FieldConfig.type; the DatePicker type ships unregistered by default via a createDatePickerInput() factory, keeping the optional dependency opt-in rather than bundled.',
      },
      {
        problem: 'Conditional visibility needs to re-evaluate on every relevant form value change without making every field in the form re-render whenever any sibling field changes.',
        solution: 'Each field calls react-hook-form\'s useWatch with `disabled: !fieldConfig.visibleIf` — only fields that declare a visibleIf rule subscribe to form-wide value changes, and every field is wrapped in React.memo, so a 50-field form does not re-render in full on every keystroke.',
      },
    ],
